...
class kiteworks_scraper:
    ...
    # initialize object:
    def __init__(self,configuration_file_path: Path=Path.cwd()/'config.txt',login_information: dict=dict()):
    ...
    def set_download_directory(self,p: Path=Path.cwd()):
        if p.is_dir():
            ...
            self.logger.log('Specified Download Directory Not Found, Using {}'.format(self.download_directory),'WARNING')
    ...
    def set_configuration_options(self,p: Path):
        if p.is_file():
            ...
            with self.configuration_options_path.open(mode='r') as f:
                for l in f:
                    if option.match(l):
                        key = option.match(l).groups()[0].lower()
                ...
                while tf:
                    ...
                    try:
                        ...
                        if keyboard.is_pressed('s'):
                            self.logger.log('STATE: {}\nPAGE LOADED: {}\nTIMER: {}\nRETRIES: {}'.format(state,document_ready,t(),retry_counter))
                        else:
                            pass
                    except:
                    ...
                    # state 6 - open first email:
                        ...
                        if emails:
                            ...
                            state =  99
                    # state 7 - read email and download attachments:
                        ...
                        try:
                            ...
                            if retry_counter >= self.maximum_retries:
                                self.logger.log('Attachments Have Not Expired','INFORMATION')
                                state = 8
                            else:
                                pass
                    # state 8 - download attachments:
                        ...
                        retry_counter += 1
                        try:
                            email_title = driver.find_element(By.XPATH,'//div[@class=\'mail-info-subject\']/span[@class=\'subject-text\']').text
                        ...
                        try:
                            ...
                            attachments = not (driver.find_element(By.XPATH,'//div[@class=\'mail-info-users\']/span[1]').text=='No attachments')
                        except:
                            attachments = True
                        if attachments:
                            try:
                                self.logger.log('Downloading Attachments -- {} from {}'.format(email_title,email_sender),'INFORMATION')
                    ...
                    # state  9 - return to inbox:
                        ...
                        try:
                            ...
                            if retry_counter>=self.maximum_retries:
                                ...
                                pass
                    # state 99 - exit and return driver attached to browser instance (for testing):
                    elif state==99:
                        tf = False
                    else:
                        pass
